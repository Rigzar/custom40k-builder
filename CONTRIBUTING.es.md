...
hay que cambiar juntas:

| Archivo | Función |
|---|---|
| `src/utils/ttsExport.ts` | Construye el payload y lo descarga (el botón **TTS** de la vista de impresión) |
| `tts/Custom40k.lua` | El mod: lee ese payload y crea las cartas |

**El mod no lleva codex, a propósito.** La app resuelve el ejército antes — con el mismo
`resolveUnitProfile` que usan la ficha de unidad y la vista de impresión — así que lo que se
exporta son stats finales, perfiles de arma finales y texto de reglas real. El Lua nunca necesita
conocer una regla de Custom40k, y actualizar un codex nunca obliga a resubir el ítem del Workshop.
Mantenelo así: si te encontrás metiendo lógica de reglas en el Lua, el arreglo va en la exportación.
...
```
npx jiti tts/test/makeExport.ts "Codex/<un ejército guardado>.json" army.json
node tts/test/run.cjs army.json          # agregá --dump para leer las cartas renderizadas
```

Su primera ejecución encontró cuatro bugs antes de que el archivo llegara al juego; el peor: en Lua
`ipairs` se detiene en el primer hueco `nil` de una tabla literal, lo que borraba en silencio la
línea de alcance/objetivo/duración de cada rezo — rompiendo justo lo que v1.70 acababa de arreglar.
Ver `tts/README.md`.
...
- **Los caps de equipo de Escaramuza** viven en `validators.ts` (dentro del bloque `eng.statCaps`). Enforzan las restricciones del suplemento de Misiones: no ganar 2+ de armadura, 4+ inv o mejor, T8+, arma de Daño 3, ni más de un ítem Único de armería — todo fundamentado en `Codex/missions_text.txt` y `core_rules_text.txt`. Añade caps nuevos aquí, no en la UI.
...
> **Mantener el canon sincronizado:** `scripts/fetch_codex.cjs` descarga cada hoja de facción directamente de los Google Sheets del autor (los enlaces están en la tabla de hipervínculos dentro de `Codex/Custom40k Core Rules.docx`) e informa, pestaña a pestaña, de cuáles de nuestras copias `Codex/*.ods` están desfasadas. Sin argumentos revisa todo; con nombres de facción, solo esas. `--apply` sobrescribe nuestra copia — a propósito es opcional y por facción, porque **sustituir un .ods significa que esa facción vuelve a estar sin auditar y debe una pasada completa**. Nunca aplicar en bloque.

> **Mantener sincronizados los DOCUMENTOS DE REGLAS:** `fetch_codex.cjs` cubre las hojas de facción y nada más, así que durante mucho tiempo los tres reglamentos podían quedarse desfasados sin ninguna señal — y ocurrió: nuestra copia de las Core Rules estaba en 1.262 mientras el documento en vivo ya iba por 1.264. `scripts/fetch_docs.cjs` lo resuelve. Descarga los documentos de Core Rules, Missions y Planetary Assault desde sus Google Docs en vivo, compara el TEXTO EXTRAÍDO en lugar de los bytes del archivo (Google reexporta un zip distinto cada vez, así que comparar bytes daría un cambio en cada ejecución), imprime la `Rules version` de cada uno y termina con código distinto de cero si alguno difiere. `--write` sustituye las copias desfasadas. Ejecutarlo junto a `fetch_codex.cjs` antes de cualquier trabajo que toque texto de reglas: una regla citada de un documento viejo es peor que no responder, porque parece correcta.

> **Auditar una facción contra su hoja:** `scripts/ods_audit.cjs "Codex/<facción>.ods" <carpeta de facción>` compara la hoja con producción unidad por unidad — filas de modelo (nombre / min / max / stats / puntos), perfiles de arma, la frase `equipped_with`, y nombres y precios de las opciones — e imprime las diferencias sin escribir nada. Un nombre de unidad como tercer argumento revisa solo esa. Hay que leer cada línea y decidirla: la hoja gana en HECHOS, producción gana en SEMÁNTICA, y aquí también salen las erratas del propio autor. Un artefacto de hoja ya resuelto: una celda `No.` como `4-19` se guarda como FECHA, así que la herramienta lee el texto formateado en vez del número de serie — eso no se "corrige" en la hoja.
