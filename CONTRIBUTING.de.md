...
> **Kanon synchron halten:** `scripts/fetch_codex.cjs` lädt jede Fraktions-Tabelle direkt aus den Google Sheets des Autors (die Links stehen in der Hyperlink-Tabelle in `Codex/Custom40k Core Rules.docx`) und meldet Tab für Tab, welche unserer `Codex/*.ods`-Kopien veraltet sind. Ohne Argumente prüft es alles, mit Fraktionsnamen nur diese. `--apply` überschreibt unsere Kopie — bewusst opt-in und pro Fraktion, denn **eine ersetzte .ods bedeutet, dass die Fraktion wieder als nicht auditiert gilt und einen vollständigen Durchgang schuldet**. Niemals alles auf einmal übernehmen.

> **Die REGELDOKUMENTE synchron halten:** `fetch_codex.cjs` deckt nur die Fraktions-Tabellen ab, deshalb konnten die drei Regelwerke lange ohne jedes Signal veralten — und taten es: unsere Core-Rules-Kopie stand auf 1.262, während das Livedokument bereits bei 1.264 war. `scripts/fetch_docs.cjs` schließt diese Lücke. Es holt die Dokumente Core Rules, Missions und Planetary Assault aus ihren Live-Google-Docs, vergleicht den EXTRAHIERTEN TEXT statt der Dateibytes (Google exportiert jedes Mal ein anderes Zip, Byte-Gleichheit würde also bei jedem Lauf eine Änderung melden), gibt die `Rules version` jedes Dokuments aus und endet mit einem Fehlercode, falls eines abweicht. `--write` ersetzt die veralteten Kopien. Vor jeder Arbeit an Regeltexten zusammen mit `fetch_codex.cjs` ausführen — eine aus einem veralteten Dokument zitierte Regel ist schlimmer als keine Antwort, weil sie richtig aussieht.

> **Fraktion gegen ihre Tabelle prüfen:** `scripts/ods_audit.cjs "Codex/<Fraktion>.ods" <Fraktionsordner>` vergleicht Tabelle und Produktion Einheit für Einheit — Modellzeilen (Name / min / max / Werte / Punkte), Waffenprofile, den `equipped_with`-Satz sowie Namen und Preise der Option-Choices — und gibt die Unterschiede aus, ohne etwas zu schreiben. Ein Einheitenname als drittes Argument prüft nur diese. Jede Zeile lesen und einzeln entscheiden: Die Tabelle gewinnt bei FAKTEN, die Produktion bei SEMANTIK, und auch die Tippfehler des Autors tauchen hier auf. Ein Tabellen-Artefakt ist bereits behandelt: eine `No.`-Zelle wie `4-19` wird als DATUM gespeichert, deshalb liest das Werkzeug den formatierten Text statt der Seriennummer — das nicht in der Tabelle "korrigieren".
