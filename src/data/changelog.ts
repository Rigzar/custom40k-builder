...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    changes: [
      'GENERAL — fix: opening Print View blanked the whole app for any army with an allied unit that carried an armory item (e.g. CSM + allies, Plaguehost, Emperor\'s Children). The print\'s armory lookup read Object.values() on an allied faction\'s armory fields, which don\'t exist on the stripped allied data, throwing and unmounting the page; the lookup now guards those fields. Also hardened a rules-of-hooks violation in the print card and wrapped Print View in an error boundary so any future render error shows a message instead of a blank screen.',
