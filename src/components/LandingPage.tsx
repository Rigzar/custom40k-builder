...
const ANNOUNCEMENT_TEXT: Record<Language, AnnouncementLang> = {
  en: {
    ...
    install: "🔫 Buying the SAME Armory item for two different models (e.g. two Boltguns on two different Chosen) showed no count at all, on any unit/Champion with no other squad option touched — a deeper root cause behind the Kill Team Veterans fix below. Fixed at the engine level, so it covers every unit and every Champion/leader hitting the same pattern, not just one datasheet.",
    ...
    line20: "🔮 Grey Knights — the Dreadnought (and 6 other vehicles) \"know Smite and Fortitude\", but Fortitude had nowhere to live and couldn't be picked. It now shows as Always Known, same as Smite. Eldar's Craftsong had the identical gap, fixed the same way — both are datasheet-only powers, printed on their own sheet instead of a shared discipline list.",
  ...
  de: {
    ...
    line20: "🔮 Grey Knights — der Dreadnought (und 6 weitere Fahrzeuge) „kennt Smite und Fortitude“, aber Fortitude hatte keinen Platz und war nicht wählbar. Es erscheint jetzt als „Immer bekannt“, genau wie Smite. Das Craftsong der Eldar hatte die identische Lücke, genauso behoben — beides reine Datenblatt-Kräfte, gedruckt auf dem eigenen Blatt statt in einer gemeinsamen Disziplinliste.",
  ...
  es: {
    ...
    line20: "🔮 Grey Knights — el Dreadnought (y otros 6 vehículos) \"conoce Smite y Fortitude\", pero Fortitude no tenía dónde vivir y no se podía elegir. Ahora aparece como \"Siempre conocido\", igual que Smite. El Craftsong de los Eldar tenía el mismo hueco, arreglado igual — ambos son poderes exclusivos de su propia ficha, impresos ahí en vez de en una lista de disciplina compartida.",
    ...
    <div className="relative mb-6">
      ...
      <div className="bg-zinc-900 border border-zinc-700 border-t-2 border-t-amber-900/80 px-5 pt-14 pb-4">
        ...
        <div className="text-[12px] text-zinc-300 leading-relaxed space-y-2">
          ...
          {/* v1.66 (2026-08-26) promoted its own top fix to title/intro/install (the previous
              v1.65 banner's title/intro/install content was folded down into line20, at the very
              end alongside the other original-launch lines1-4 — same batch, just recovered from
              the header slots instead of a numbered line). line17/line16/line15/line14/line13/
              line12/line11/line10/line9/line8/line7/line6/line5 are prepend slots: a follow-up
              batch of fixes added after the v1.65 release leads with the newest slot, without
              renumbering anything below it. 2026-08-24: line7 (Save-bonus engine fix), line8
              (Tyranids/GK codex sync), line9 (Tyrant Guard Brood HQ-slot toggle), line10 (Legion/
              Taghmata Legacy-armory access), line11 (HH/Legio Titanicus armory-scope + Crusade
              weapon picker) and line12 (Cheat Sheets fixes + home-screen button) added this way.
              2026-08-25: line13 (Inquisitor stale mandatory-HQ warning removed), line14
              (archetypeArmory/authority tabs now sectioned + picker-aware), line15 (non-Champion
              Armory weapon purchase count + Stormtrooper plasma gun typo), line16 (Henchman
              Warband weapon list not gated by which specialists are present) and line17 (Rough
              Riders' 4 mount upgrades shown unconditionally) added the same way. */}
          {[tx.line17, tx.line16, tx.line15, tx.line14, tx.line13, tx.line12, tx.line11, tx.line10, tx.line9, tx.line8, tx.line7, tx.line6, tx.line5, tx.line1, tx.line2, tx.line3, tx.line4, tx.line20]
            .filter(Boolean)
