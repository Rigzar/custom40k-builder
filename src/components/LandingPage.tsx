...
const ANNOUNCEMENT_KEY = 'c40k_announcement_v175a_dismissed';
...
const ANNOUNCEMENT_TEXT: Record<Language, AnnouncementLang> = {
  en: {
    title: "v1.75: the rules we had been quoting from a stale copy",
    intro: "Our own copy of the Core Rules was two versions behind the live document, and nobody could have noticed — the freshness check only ever covered the faction sheets. Fixing that turned up five rules that were in the rulebook and written down nowhere in the app, and a run of reports about the archetypes that turn a loyal army traitor.",
    install: "",
    line1: "📕 THE CORE RULES ARE NOW 1.264, AND NO RULE ACTUALLY CHANGED — an earlier version of this note said UNYIELDING had lost its double-shot option, and that was our misreading. The author: “Unyielding can fire up to full range normally always, or up to half range double, like every other unit with Rapid Fire. It always worked like this, but was a victim of the weird wording.” Nothing on your cards moved. What DID move is wording, now matched in the glossary, the Field Manual, both cheat sheets in all three languages and the wiki: a save you cannot possibly make is skipped rather than rolled, a fully immobilised vehicle cannot move at all (Advance and Charge included), and reinforcements arrive during their activation. AND READING THE WHOLE RULEBOOK AGAINST OUR OWN SUMMARIES TURNED UP FIVE RULES WE NEVER WROTE DOWN: the wiki now has MELEE OVER TERRAIN, TRANSPORTS in full, VEHICLE DESTRUCTION, MODIFYING PROFILE VALUES and PROFILE VALUE TESTS — that last one is the rule behind every “must pass a Leadership value test” in every codex, and it existed in no summary at all. The cheat sheets gained four more.",
    line2: "☠️ THE ARCHETYPES THAT TURN YOU TRAITOR ALL WORK NOW. DARK MECHANICUM AND TRAITOR GUARD CAN BUY MARKS OF CHAOS — reported by DirtyHerbert, confirmed by the author (the codex says ALL units, not just Chaos Space Marines), and there was simply no button: the Mark picker only appeared for a faction with the Animosity of the Gods table, which only the two Chaos codices have. Every unit in such an army can take one now, vehicles included, at the codex price. DAEMON WEAPONS WORK ON THEM TOO — reported by jjbooklord42: you could buy the gateway from the borrowed Chaos Armory and nothing ever asked which ability it granted. Fixed, and Dark Mechanicum correctly loses the god-specific ones its codex entry never granted. AND SPACE MARINE RENEGADES CAN AT LAST TAKE ITS ONE CHAOS TRAIT: the archetype always promised it and the app did nothing with it, so we asked rather than guess — the author ruled that a Renegades army still has two Traits with one of them from the Chaos list, and added a line to the codex making every Space Marine count as having the CHAOS SPACE MARINE keyword.",
    contrib: "👁️ Found something wrong? The in-app bug report form works — unit, engagement, archetype and a picture.",
  },
  de: {
    title: "v1.75: die Regeln, die wir aus einer veralteten Kopie zitiert haben",
    intro: "Unsere eigene Kopie der Core Rules war zwei Versionen hinter dem Livedokument, und niemand hätte es merken können — die Aktualitätsprüfung deckte nur die Fraktionstabellen ab. Beim Beheben kamen fünf Regeln zutage, die im Regelwerk stehen und in der App nirgends, dazu eine Reihe Meldungen zu den Archetypen, die eine loyale Armee zum Verräter machen.",
    install: "",
    line1: "📕 DIE CORE RULES SIND JETZT 1.264, UND ES HAT SICH KEINE REGEL GEÄNDERT — eine frühere Fassung dieser Notiz behauptete, UNYIELDING habe seinen Doppelschuss verloren; das war unser Lesefehler. Der Autor: „Unyielding darf immer bis zur vollen Reichweite feuern oder auf halbe Reichweite doppelt, wie jede andere Einheit mit Rapid Fire. Es funktionierte immer so, war aber Opfer der seltsamen Formulierung.“ Auf euren Karten hat sich nichts verändert. Geändert hat sich die Formulierung, jetzt nachgezogen in Glossar, Field Manual, beiden Cheat Sheets in allen drei Sprachen und im Wiki: ein unmöglicher Rettungswurf wird übersprungen statt gewürfelt, ein vollständig immobilisiertes Fahrzeug kann sich überhaupt nicht bewegen (Advance und Charge eingeschlossen), und Verstärkungen kommen während ihrer Aktivierung an. UND DAS DURCHLESEN DES GANZEN REGELWERKS GEGEN UNSERE EIGENEN ÜBERSICHTEN FÖRDERTE FÜNF REGELN ZUTAGE, DIE WIR NIE AUFGESCHRIEBEN HATTEN: im Wiki stehen jetzt MELEE OVER TERRAIN, TRANSPORTS vollständig, VEHICLE DESTRUCTION, MODIFYING PROFILE VALUES und PROFILE VALUE TESTS — letzteres ist die Regel hinter jedem „Leadership value test“ in jedem Codex und stand in keiner Übersicht. Die Cheat Sheets haben vier weitere bekommen.",
    line2: "☠️ DIE VERRÄTER-ARCHETYPEN FUNKTIONIEREN JETZT ALLE. DARK MECHANICUM UND TRAITOR GUARD KÖNNEN MALE DES CHAOS KAUFEN — von DirtyHerbert gemeldet, vom Autor bestätigt (laut Codex ALLE Einheiten, nicht nur Chaos Space Marines), und es gab schlicht keinen Knopf: die Mal-Auswahl erschien nur für Fraktionen mit der Tabelle ‚Animosity of the Gods‘, die nur die beiden Chaos-Codices haben. Jetzt kann jede Einheit einer solchen Armee eines nehmen, Fahrzeuge eingeschlossen, zum Codexpreis. DÄMONENWAFFEN FUNKTIONIEREN EBENFALLS — von jjbooklord42 gemeldet: man konnte den Zugang in der geliehenen Chaos-Armory kaufen, und niemand fragte je, welche Fähigkeit er gewährt. Behoben, und Dark Mechanicum verliert korrekt die gottspezifischen, die sein Codexeintrag nie gewährt hat. UND SPACE MARINE RENEGADES KANN ENDLICH SEINEN EINEN CHAOS-TRAIT NEHMEN: der Archetyp versprach ihn immer, die App tat nichts damit, also haben wir gefragt statt geraten — der Autor entschied, dass eine Renegades-Armee weiterhin zwei Traits hat, einer davon aus der Chaos-Liste, und ergänzte den Codex um eine Zeile, die jeden Space Marine als Träger des Schlüsselworts CHAOS SPACE MARINE zählen lässt.",
    contrib: "👁️ Etwas falsch? Das Bug-Report-Formular in der App funktioniert — Einheit, Engagement, Archetyp und ein Bild.",
  },
  es: {
    title: "v1.75: las reglas que estábamos citando de una copia vieja",
    intro: "Nuestra copia de las Core Rules iba dos versiones por detrás del documento en vivo, y nadie podía haberlo notado: la comprobación de actualidad solo cubría las hojas de facción. Arreglarlo sacó cinco reglas que están en el reglamento y en la app no estaban escritas en ninguna parte, más una tanda de reportes sobre los arquetipos que vuelven traidor a un ejército leal.",
    install: "",
    line1: "📕 LAS CORE RULES YA ESTÁN EN 1.264, Y NO CAMBIÓ NINGUNA REGLA — una versión anterior de esta nota decía que UNYIELDING había perdido su disparo doble, y era un error de lectura nuestro. El autor: “Unyielding puede disparar siempre hasta el alcance completo, o a media distancia el doble, como cualquier otra unidad con Rapid Fire. Siempre funcionó así, pero fue víctima de una redacción rara.” En tus fichas no se movió nada. Lo que sí cambió es redacción, ya igualada en el glosario, el Field Manual, ambos cheat sheets en los tres idiomas y la wiki: una salvación imposible se omite en vez de tirarse, un vehículo completamente inmovilizado no puede moverse en absoluto (Advance y Charge incluidos), y los refuerzos llegan durante su activación. Y LEER EL REGLAMENTO ENTERO CONTRA NUESTROS PROPIOS RESÚMENES SACÓ CINCO REGLAS QUE NUNCA HABÍAMOS ESCRITO: la wiki ya tiene MELEE OVER TERRAIN, TRANSPORTS completo, VEHICLE DESTRUCTION, MODIFYING PROFILE VALUES y PROFILE VALUE TESTS — esta última es la regla detrás de cada “Leadership value test” de cada códex, y no estaba en ningún resumen. Los cheat sheets ganaron cuatro más.",
    line2: "☠️ LOS ARQUETIPOS QUE TE VUELVEN TRAIDOR YA FUNCIONAN TODOS. DARK MECHANICUM Y TRAITOR GUARD PUEDEN COMPRAR MARCAS DEL CAOS — reportado por DirtyHerbert y confirmado por el autor (el códex dice TODAS las unidades, no solo Chaos Space Marines), y simplemente no había botón: el selector de Marca solo aparecía para una facción con la tabla de Animosity of the Gods, que solo tienen los dos códex del Caos. Ahora cualquier unidad de un ejército así puede llevar una, vehículos incluidos, al precio del códex. LAS ARMAS DEMONÍACAS TAMBIÉN — reportado por jjbooklord42: podías comprar el acceso en la armería del Caos prestada y nadie te preguntaba nunca qué habilidad concedía. Arreglado, y Dark Mechanicum pierde correctamente las específicas de dios que su entrada nunca concedía. Y RENEGADES DE SPACE MARINES YA PUEDE COGER SU ÚNICO RASGO DEL CAOS: el arquetipo siempre lo prometía y la app no hacía nada con él, así que preguntamos en vez de adivinar — el autor decidió que un ejército Renegades sigue teniendo dos Rasgos, uno de ellos de la lista del Caos, y añadió una línea al códex que hace que todo Space Marine cuente con la keyword CHAOS SPACE MARINE.",
