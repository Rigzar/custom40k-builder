...
const ANNOUNCEMENT_KEY = 'c40k_announcement_v156c_dismissed';
...
const ANNOUNCEMENT_TEXT: Record<Language, AnnouncementLang> = {
  en: {
    ...
    install: '',
    line6: '🔎 Inquisition (GitHub #74) — a Henchman Warband or Daemonhosts unit with a Daemonhost can now pick its power: the "Malefic (Daemonhosts only)" discipline was missing from the app and is back with all six powers. · 🔢 Weapon quantities were corrected everywhere: taking the same option twice (two Reaper autocannons on a Defiler) now reads "2x", loadouts written in words ("two Plague spewers") are counted, and weapons with several firing modes are now shown like the codex writes them — the weapon named once with its quantity ("2x Knight melee weapon") and its modes listed underneath ("— Strike", "— Sweep"), instead of repeating the whole name on every mode.',
    ...
    line5: '🔧 Everyone — a unit that starts with two of the same weapon now shows "2x" in its profile; the Plague ammunition only affects the champion who buys it (GitHub #73); the points limit can be typed freely again on phones; and the "which unit does this character join" picker is gone, since joining is decided during the game. · A weapon a unit already has AND that the Armory also sells (the Plague Marines\' Plasma gun, the Orks\' Choppa, the Eldar Shuriken cannon, ~160 units in all) was being duplicated when the champion bought it, which also made the squad\'s own copies vanish from the squad row — a squad with two Plasma guns showed no Plasma guns at all. Fixed everywhere. · And 45 more datasheets were showing a weapon nobody had paid for, because the option that buys it names it only in its own text (the “additional Storm bolter” on the Land Raider, Predator, Razorback, Vindicator, Whirlwind and Stormraven, the Wraithlord’s Ghostglaive, the Sentinel’s chainsaw, the Sub-Commander’s Markerlight, the Big Mek’s Shokk attack gun) or names the crew as well as the gun (the Warkopta’s “Kustom mega-blasta with grot gunner”). They now appear only once you take them. · 🎖 Imperial Guard (GitHub #75) — the Veteran Company archetype showed almost no units to choose from. Its codex says every unit GAINS access to veteran abilities and must take one; the app read that as a restriction and kept only the units that already had that access, which in the Guard is one. The whole roster is back, and every unit in a Veteran Company can buy its veteran ability. · A Champion that buys a weapon from the Armory now shows it — when the squad also has that weapon on its datasheet, the one you paid for used to appear nowhere. · On a phone, choosing a faction or pressing “Add Troops” now starts at the top of the new screen instead of halfway down it, and the points box tells you how many points you have left.',
  ...
  de: {
    ...
    install: '',
    line6: '🔎 Inquisition (GitHub #74) — eine Henchman Warband oder Daemonhosts-Einheit mit einem Daemonhost kann ihre Kraft jetzt auswählen: die Disziplin „Malefic (nur Daemonhosts)" fehlte in der App und ist mit allen sechs Kräften zurück. · 🔢 Waffenanzahlen wurden überall korrigiert: dieselbe Option zweimal genommen (zwei Reaper autocannons beim Defiler) zeigt jetzt „2x", in Worten geschriebene Ausrüstung („two Plague spewers") wird mitgezählt, und Waffen mit mehreren Feuermodi werden jetzt so dargestellt, wie der Codex sie schreibt — die Waffe einmal mit ihrer Anzahl („2x Knight melee weapon") und darunter ihre Modi („— Strike", „— Sweep"), statt den ganzen Namen bei jedem Modus zu wiederholen.',
    ...
    line5: '🔧 Für alle — eine Einheit mit zwei gleichen Waffen zeigt jetzt „2x" im Profil; die Plague ammunition wirkt nur auf den Champion, der sie kauft (GitHub #73); das Punktelimit lässt sich am Handy wieder frei eintippen; und die Auswahl „welcher Einheit schließt sich dieser Charakter an" ist entfallen, da das im Spiel entschieden wird. · Eine Waffe, die eine Einheit bereits besitzt und die es AUCH in der Rüstkammer gibt (Plasma gun der Plague Marines, Choppa der Orks, Shuriken cannon der Eldar, insgesamt ~160 Einheiten), wurde doppelt eingefügt, wenn der Champion sie kaufte — dabei verschwanden auch die eigenen Exemplare der Einheit aus ihrer Zeile: eine Einheit mit zwei Plasma guns zeigte gar keine an. Überall behoben. · Und 45 weitere Datenblätter zeigten eine Waffe, die niemand bezahlt hatte, weil die zugehörige Option sie nur im eigenen Text nennt (der „zusätzliche Storm bolter" bei Land Raider, Predator, Razorback, Vindicator, Whirlwind und Stormraven, das Ghostglaive des Wraithlord, die Kettensäge des Sentinel, das Markerlight des Sub-Commander, die Shokk attack gun des Big Mek) oder neben der Waffe auch die Crew nennt („Kustom mega-blasta with grot gunner" beim Warkopta). Sie erscheinen jetzt erst, wenn man sie nimmt. · 🎖 Imperial Guard (GitHub #75) — der Archetyp Veteran Company zeigte fast keine Einheiten zur Auswahl. Sein Codex sagt, dass ALLE Einheiten Zugang zu Veteranenfähigkeiten ERHALTEN und eine wählen müssen; die App las das als Einschränkung und behielt nur die Einheiten, die diesen Zugang schon hatten — bei der Guard genau eine. Die ganze Auswahl ist zurück. · Kauft ein Champion eine Waffe in der Rüstkammer, wird sie jetzt auch angezeigt. · Am Handy beginnt eine neue Seite jetzt oben statt in der Mitte, und die Punkteanzeige nennt die verbleibenden Punkte.',
  ...
  es: {
    ...
    install: '',
    line6: '🔎 Inquisition (GitHub #74) — una Henchman Warband o unos Daemonhosts con un Daemonhost ya pueden elegir su poder: la disciplina "Malefic (solo Daemonhosts)" faltaba en la app y vuelve con sus seis poderes. · 🔢 Se corrigieron las cantidades de armas en todas partes: coger la misma opción dos veces (dos Reaper autocannon en el Defiler) ya muestra "2x", los loadouts escritos en palabras ("two Plague spewers") se cuentan, y las armas con varios modos de disparo se muestran como las escribe el códex — el arma nombrada una vez con su cantidad ("2x Knight melee weapon") y debajo sus modos ("— Strike", "— Sweep"), en vez de repetir el nombre entero en cada modo.',
    ...
    line5: '🔧 Para todos — una unidad que empieza con dos armas iguales ahora muestra "2x" en su perfil; la Plague ammunition solo afecta al campeón que la compra (GitHub #73); el límite de puntos vuelve a poder escribirse en el móvil; y el selector de "a qué unidad se une este personaje" desaparece, porque eso se decide durante la partida. · Un arma que la unidad ya tiene y que ADEMÁS vende la armería (el Plasma gun de los Plague Marines, el Choppa de los Orkos, el Shuriken cannon eldar, unas 160 unidades) se duplicaba al comprarla el campeón, y de paso las copias del escuadrón desaparecían de su fila: un escuadrón con dos Plasma gun no mostraba ninguno. Corregido en todas partes. · Y otras 45 fichas mostraban un arma que nadie había pagado, porque la opción que la compra solo la nombra en su propio texto (el “Storm bolter adicional” del Land Raider, Predator, Razorback, Vindicator, Whirlwind y Stormraven, el Ghostglaive del Wraithlord, la motosierra del Sentinel, el Markerlight del Sub-Commander, el Shokk attack gun del Big Mek) o nombra también a la dotación además del arma (el “Kustom mega-blasta with grot gunner” del Warkopta). Ahora solo aparecen cuando las coges. · 🎖 Imperial Guard (GitHub #75) — el arquetipo Veteran Company no dejaba elegir casi ninguna unidad. Su códex dice que todas las unidades GANAN acceso a habilidades de veterano y deben coger una; la app lo leía como una restricción y se quedaba solo con las que ya tenían ese acceso, que en la Guardia es una. Vuelve el catálogo entero. · Si un campeón compra un arma en la armería, ahora se ve. · En el móvil, cada pantalla nueva empieza arriba y el recuadro de puntos dice cuántos te quedan.',
