...
const ANNOUNCEMENT_KEY = 'c40k_announcement_v158f_dismissed';
...
const ANNOUNCEMENT_TEXT: Record<Language, AnnouncementLang> = {
  en: {
    ...
    contrib: '❓ Two open reports we cannot fix without you. (1) A Space Marine Terminator Squad listing weapons it never took, and a Lieutenant whose Plasma pistol is missing from the printed card. (2) Tyranid Basic/Advanced Biomorphs stopping at 16 — we think that 16 is your brood’s model count rather than a limit, but which it is changes the fix. If either was you, please answer on our Discord: your list’s .json (Review → ↓ JSON) plus a screenshot settle both in minutes. Both are listed under Known Issues meanwhile. · 👁️ Reporting something else? Be specific — which unit, engagement and archetype, with a screenshot. Vague reports cannot be reproduced, so they cannot be fixed.',
  },
  de: {
    ...
    contrib: '❓ Zwei offene Meldungen, die wir ohne euch nicht beheben können. (1) Eine Space-Marine-Terminatoreinheit listet Waffen, die sie nie gewählt hat, und beim Lieutenant fehlt die Plasmapistole auf der gedruckten Karte. (2) Tyraniden-Biomorphe (Basic/Advanced) stoppen bei 16 — wir vermuten, 16 ist die Modellzahl eurer Brut und keine Grenze, aber was davon zutrifft, ändert die Lösung. Wenn das jemand von euch war: bitte auf unserem Discord antworten — die .json eurer Liste (Prüfung → ↓ JSON) und ein Screenshot klären beides in Minuten. Bis dahin stehen beide unter „Bekannte Probleme“. · 👁️ Etwas anderes melden? Bitte konkret: welche Einheit, welches Engagement, welcher Archetyp, mit Screenshot. Vage Meldungen lassen sich nicht nachstellen und daher nicht beheben.',
  },
  es: {
    ...
    contrib: '❓ Dos reportes abiertos que no podemos arreglar sin vosotros. (1) Una Terminator Squad de Space Marines que lista armas que nunca eligió, y un Lieutenant al que le falta la pistola de plasma en la ficha impresa. (2) Los biomorfos tyránidos (Basic/Advanced) se paran en 16 — creemos que ese 16 es el número de modelos de vuestra brood y no un tope, pero cuál de los dos sea cambia el arreglo. Si fuisteis vosotros, contestad en nuestro Discord: el .json de vuestra lista (Revisión → ↓ JSON) y una captura lo resuelven en minutos. Mientras tanto están en Problemas conocidos. · 👁️ ¿Reportas otra cosa? Sé concreto: qué unidad, qué engagement y qué arquetipo, con captura. Los reportes vagos no se pueden reproducir, así que no se pueden arreglar.',
  },
};
