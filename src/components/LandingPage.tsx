...
const ANNOUNCEMENT_KEY = 'c40k_announcement_v176f_dismissed';
...
//
//   line5 = 2026-09-20, still v1.76 (Rigzar: "misma version"), so the key goes to v176d.
//           A player reported that psychic powers never show their details anywhere; they were
//           right, and it was two faults at once. Leads with what the player sees, not the
//           mechanism. The Battle Fortress and the red × ride along. The Core Book audit
//           finishing is NOT in the banner: nothing about it changes a card.
//
//   line6 = the two pushes that followed line5, announced together on ONE key bump (v176d ->
//           v176e) rather than reopening the card three times in a day: the five deployment
//           rules a player found unbuyable, the allied-detachment Objective secured! fix, and
//           GH#136. Still v1.76.
//
//   line7 = the faction-by-faction audit's first five findings. Leads with the two that move
//           NUMBERS on a saved list (Tau points, Sororitas saves) and says so plainly, because
//           those are the ones a player has to act on. Still v1.76; key v176e -> v176f.
//
...
const ANNOUNCEMENT_TEXT: Record<Language, AnnouncementLang> = {
  en: {
    ...
    line4: "⚔️ ALLIES: SEVEN ARCHETYPES THAT REWRITE THE ALLY MATRIX NOW ACTUALLY DO. TRAITOR GUARD are “treated like Chaos Space Marines in the Ally matrix” in so many words, so with a Chaos army they are BATTLE BROTHERS, not the Desperate Allies the app showed. Same for SPACE MARINE RENEGADES; BROOD BROTHERS read the Genestealer Cults row and GUE’VESA the Tau row (both Battle Brothers with the faction they copy — the app used to say Allies of Convenience); DARK MECHANICUM are Allies of Convenience with Chaos Space Marines; and YNNARI are Battle Brothers with the other Aeldari codex in both directions. Half the bug was that the app only read the PRIMARY army’s archetype, and every one of these sentences is printed on the detachment being brought IN.  🛠️ AND SIX REPORTS FROM THE LAST DAY, five of them Necron. A NECRON ARMY GOT NO DEDICATED TRANSPORT SLOTS AT ALL (GH#131): the rule grants one per “Infantry” selection, and every Necron datasheet reads “Infantry, Necron”, which the app was not accepting as Infantry. SCARABS SELECTED AS TROOPS NOW COUNT TOWARDS THE 25% (GH#130) — they still cannot fill one of your two required Troops slots, which is the other half of their rule. A HEXMARK DESTROYER NO LONGER CONJURES A SECOND AOP (GH#133): its Royal Assassin exemption was applied to the slot count but not to the check that decides whether you need a second plan, so an exempt unit doubled your limits. A +1 STRENGTH RELIC NOW WORKS ON A MELEE WEAPON (GH#134) — those print “+2” rather than a number, and the app only recognised numbers, so the bonus did nothing on 103 weapons across five factions. A CANOPTEK SPYDER SQUADRON CAN BUY MORE THAN ONE ITEM (GH#132). AND AN ALLIED TRAITOR GUARD DETACHMENT CAN BUILD A PLATOON (GH#135), which is the same fix as above.",
    line5: "✨ YOUR PSYCHIC POWERS NOW TELL YOU WHAT THEY DO. Reported by a player, and it was right: a power showed as a bare name — no range, no cast value, no target, no duration, no effect — on the unit card, on the printed sheets and in the battle view. Two separate faults. The six GENERAL DISCIPLINES every psyker can pick from (Biomancy, Divination, Pyromancy, Telekinesis, Telepathy and the always-known list) are not stored with your faction, so ALL 31 of those powers came back “not found” and printed as a name, in every faction — while a power from your own codex one line above printed everything. And the unit card never ran POWERS through the formatter at all: prayers and pacts got their full line months ago and powers were left out. All 808 selectable powers now resolve, and THE BATTLE VIEW carries the details too — the one screen you read mid-game was the worst off. ⚔️ ORKS: THE BATTLE FORTRESS IS SELECTABLE. Escalation gives the Orks three Lords of War and the app only ever offered two — the datasheet was complete and sitting in a file nothing loaded. 280 points, AV 14/13/11, 5 Hull Points, and a transport capacity of 30 that drops by 10 for each big gun you bolt on. 🔴 And the × that closes this banner is RED now, because Unwise kept losing track of it.",
    line6: "⚔️ A RULE FIVE FACTIONS SELL YOU AND NOBODY COULD BUY. Truesight asked how to spend the point per Wound for the Eldar WEBWAY STRIKE, and the answer was that he could not — it was printed in the codex and the app only ever showed it as text. The same hole in five places: ELDAR and HARLEQUINS “Webway strike”, DARK ELDAR “Webway raid”, CUSTODES “Lightning strike” and ORKS “Tellyporta”. Every unit that qualifies now has a tick box showing what it costs for THAT unit at its current size — resize the squad and the price follows — the card gains the deployment rule, and the army limit is enforced: one unit per STARTED 1000 points, so three at 2500. 🛡️ AND AN ALLIED DETACHMENT NO LONGER GETS “OBJECTIVE SECURED!”, which the Core Rules forbid outright. Your own army is unaffected, and neither is a supplement like an Assassin. 💎 A RELIC YOU MAY TAKE TWICE CAN NOW TAKE A SECOND, DIFFERENT ENHANCEMENT (GH#136). Reported on the NECRONS TOMB WORLD RELIC: the enhancement picker vanished once you had bought one, and “+ Add another” simply repeated the first choice. It stays open now, and the codex rule is enforced with it — “every enhancement is unique per army” — so one already taken anywhere in your list is greyed out and marked. 19 relics in the game work this way, not just the Necron one.",
    line7: "📜 FIVE RULES YOUR CODEX GRANTS THAT THE APP WAS NOT APPLYING — TWO OF THEM CHANGE A SAVED LIST, SO CHECK YOURS. ADEPTUS SORORITAS: “Shield of Faith: the model receives a 6+ ward save” is printed in the Index and the datasheets carry only the NAME, so there was no number for us to find — 23 OF 28 SORORITAS DATASHEETS carry it and THIRTEEN SHOWED NO WARD SAVE AT ALL. They show 6+ now; the ten that already had a better save keep it. TAU EMPIRE: TACTICAL PHILOSOPHIES WAS FREE. The sheet prices it in words — “10p per 500p game size” — where every other price in the game is a number, so it went in for nought; it now costs 20 at 1000 points and 50 at 2500, and your list total will go up. ALSO: the TAU KROOT SHAMAN charged its +10, turned the model into a psyker and never opened a power picker (its Biomancy and Divination are shared disciplines, and we only showed the psychic panel to factions with one of their own). GREY KNIGHTS: the NEMESIS WARDING STAVE gives the whole squad “Parry”, on all twelve datasheets that can buy it. IMPERIAL GUARD: the extra Special weapon is only yours IF NO HEAVY WEAPONS TEAM IS FORMED — taking both is now flagged. All five came out of a new sweep that asks, of every rule written in every codex, whether anything in the app actually acts on it.",
  ...
  de: {
    ...
    line4: "⚔️ VERBÜNDETE: SIEBEN ARCHETYPEN, DIE DIE ALLY-MATRIX UMSCHREIBEN, TUN ES JETZT AUCH. TRAITOR GUARD werden wörtlich „wie Chaos Space Marines in der Ally-Matrix behandelt“, also mit einer Chaos-Armee BATTLE BROTHERS statt der angezeigten Desperate Allies. Ebenso SPACE MARINE RENEGADES; BROOD BROTHERS lesen die Genestealer-Cults-Zeile und GUE’VESA die Tau-Zeile (beide Battle Brothers, vorher „Allies of Convenience“); DARK MECHANICUM sind Allies of Convenience mit Chaos Space Marines; YNNARI sind in beide Richtungen Battle Brothers mit dem anderen Aeldari-Codex. Die Hälfte des Fehlers: die App las nur den Archetyp der PRIMÄRarmEE, und diese Sätze stehen jeweils beim Detachment, das mitgebracht wird.  🛠️ UND SECHS MELDUNGEN VOM LETZTEN TAG, fünf davon Necrons. EINE NECRON-ARMEE BEKAM ÜBERHAUPT KEINE DEDICATED-TRANSPORT-SLOTS (GH#131): die Regel gibt einen pro „Infantry“-Auswahl, und jedes Necron-Datasheet liest „Infantry, Necron“, was die App nicht als Infantry akzeptierte. ALS TROOPS GEWÄHLTE SCARABS ZÄHLEN JETZT ZU DEN 25% (GH#130) — einen der beiden Pflicht-Troops-Slots füllen sie weiterhin nicht. EIN HEXMARK DESTROYER ERZWINGT KEINEN ZWEITEN AOP MEHR (GH#133). EIN +1-STÄRKE-RELIKT WIRKT JETZT AUF EINE NAHKAMPFWAFFE (GH#134) — die drucken „+2“ statt einer Zahl, und die App erkannte nur Zahlen; betroffen waren 103 Waffen in fünf Fraktionen. EIN CANOPTEK-SPYDER-TRUPP KANN MEHR ALS EINEN GEGENSTAND KAUFEN (GH#132). UND EIN VERBÜNDETES TRAITOR-GUARD-DETACHMENT KANN EIN PLATOON BILDEN (GH#135).",
    line5: "✨ EURE PSIKRÄFTE SAGEN JETZT, WAS SIE TUN. Von einem Spieler gemeldet, und er hatte recht: eine Kraft stand nur als Name da — ohne Reichweite, Wirkwert, Ziel, Dauer oder Effekt — auf der Einheitenkarte, auf den Druckblättern und in der Battle View. Zwei getrennte Fehler. Die sechs ALLGEMEINEN DISZIPLINEN, aus denen jeder Psioniker wählen darf (Biomantie, Divination, Pyromantie, Telekinese, Telepathie und die immer bekannte Liste), liegen nicht bei eurer Fraktion, also kamen ALLE 31 dieser Kräfte als „nicht gefunden“ zurück und wurden als bloßer Name gedruckt — in jeder Fraktion — während eine Kraft aus dem eigenen Codex eine Zeile darüber alles anzeigte. Und die Einheitenkarte schickte KRÄFTE nie durch den Formatierer: Gebete und Pakte bekamen ihre volle Zeile vor Monaten, Kräfte blieben außen vor. Alle 808 wählbaren Kräfte lösen jetzt auf, und DIE BATTLE VIEW zeigt die Angaben ebenfalls — ausgerechnet der Bildschirm, den ihr mitten im Spiel lest, war am schlechtesten dran. ⚔️ ORKS: DIE BATTLE FORTRESS IST WÄHLBAR. Escalation gibt den Orks drei Lords of War, und die App bot nur zwei an — das Datenblatt war vollständig und lag in einer Datei, die nichts geladen hat. 280 Punkte, PW 14/13/11, 5 Strukturpunkte und eine Transportkapazität von 30, die pro großer Kanone um 10 sinkt. 🔴 Und das × zum Schließen dieses Banners ist jetzt ROT, weil Unwise es ständig gesucht hat.",
    line6: "⚔️ EINE REGEL, DIE FÜNF FRAKTIONEN ANBIETEN UND NIEMAND KAUFEN KONNTE. Truesight fragte, wie man den Punkt pro Wunde für den Eldar-WEBWAY STRIKE ausgibt — gar nicht: die Regel stand im Codex und die App zeigte sie nur als Text. Dieselbe Lücke an fünf Stellen: ELDAR und HARLEQUINS „Webway strike“, DARK ELDAR „Webway raid“, CUSTODES „Lightning strike“ und ORKS „Tellyporta“. Jede geeignete Einheit hat jetzt ein Kästchen mit dem Preis für GENAU diese Einheit in ihrer aktuellen Größe — Trupp ändern und der Preis folgt — die Karte erhält die Aufstellungsregel, und das Limit gilt: eine Einheit je ANGEFANGENE 1000 Punkte, also drei bei 2500. 🛡️ UND EINE VERBÜNDETE ABORDNUNG BEKOMMT „OBJECTIVE SECURED!“ NICHT MEHR, was die Core Rules ausdrücklich verbieten. Die eigene Armee ist nicht betroffen, ein Supplement wie ein Assassine ebenso wenig. 💎 EIN ZWEIMAL WÄHLBARES RELIKT KANN JETZT EINE ZWEITE, ANDERE VERBESSERUNG NEHMEN (GH#136). Gemeldet am NECRONS TOMB WORLD RELIC: die Auswahl verschwand nach dem ersten Kauf, und „+ Weiteres hinzufügen“ wiederholte einfach die erste Wahl. Sie bleibt nun offen, und die Codexregel gilt mit ihr — „jede Verbesserung ist einmalig pro Armee“ — bereits vergebene sind ausgegraut und markiert. 19 Relikte im Spiel funktionieren so, nicht nur das der Necrons.",
    line7: "📜 FÜNF REGELN AUS EUREM CODEX, DIE DIE APP NICHT ANWANDTE — ZWEI DAVON ÄNDERN EINE GESPEICHERTE LISTE, ALSO PRÜFT SIE. ADEPTUS SORORITAS: „Shield of Faith: das Modell erhält eine 6+ Rettung“ steht im Index, und die Datenblätter tragen nur den NAMEN — 23 VON 28 BLÄTTERN haben sie und DREIZEHN ZEIGTEN GAR KEINE RETTUNG. Jetzt 6+; die zehn mit einer besseren behalten sie. TAU EMPIRE: TACTICAL PHILOSOPHIES WAR KOSTENLOS. Das Blatt bepreist es in Worten — „10p pro 500p Spielgröße“ — wo jeder andere Preis eine Zahl ist; es kostet nun 20 bei 1000 und 50 bei 2500 Punkten, euer Listentotal steigt. AUSSERDEM: der TAU KROOT SHAMAN kostete seine +10, wurde zum Psioniker und bekam nie eine Kraftauswahl (Biomantie und Divination sind gemeinsame Disziplinen, und wir zeigten das Psi-Panel nur Fraktionen mit eigener). GREY KNIGHTS: der NEMESIS WARDING STAVE gibt dem ganzen Trupp „Parry“, auf allen zwölf Datenblättern, die ihn kaufen können. IMPERIAL GUARD: die zusätzliche Spezialwaffe gibt es nur, WENN KEIN SCHWERES WAFFENTEAM GEBILDET WIRD — beides zusammen wird jetzt gemeldet. Alle fünf stammen aus einem neuen Durchgang, der für jede Regel jedes Codex fragt, ob die App überhaupt etwas damit tut.",
  ...
  es: {
    ...
    line4: "⚔️ ALIADOS: SIETE ARQUETIPOS QUE REESCRIBEN LA MATRIZ DE ALIADOS YA LO HACEN DE VERDAD. TRAITOR GUARD dice literalmente que “se tratan como Chaos Space Marines en la matriz”, así que con un ejército del Caos son BATTLE BROTHERS, no los Desperate Allies que mostraba la app. Igual SPACE MARINE RENEGADES; BROOD BROTHERS leen la fila de Genestealer Cults y GUE’VESA la de Tau (ambos Battle Brothers con la facción que copian — antes decía Allies of Convenience); DARK MECHANICUM son Allies of Convenience con Chaos Space Marines; e YNNARI son Battle Brothers con el otro códex aeldari en los dos sentidos. La mitad del fallo era que la app solo miraba el arquetipo del ejército PRINCIPAL, y estas frases están impresas en el destacamento que traes.  🛠️ Y SEIS REPORTES DEL ÚLTIMO DÍA, cinco de Necrones. UN EJÉRCITO NECRÓN NO OBTENÍA NINGUNA RANURA DE TRANSPORTE DEDICADO (GH#131): la regla da una por selección de “Infantry”, y cada ficha necrona pone “Infantry, Necron”, que la app no aceptaba como Infantry. LOS SCARABS TOMADOS COMO TROOPS YA CUENTAN PARA EL 25% (GH#130) — siguen sin poder cubrir una de tus dos ranuras obligatorias de Troops. UN HEXMARK DESTROYER YA NO INVOCA UN SEGUNDO AOP (GH#133). UNA RELIQUIA DE +1 FUERZA YA FUNCIONA EN UN ARMA DE CUERPO A CUERPO (GH#134) — esas imprimen “+2” en vez de un número, y la app solo reconocía números: 103 armas en cinco facciones. UN ESCUADRÓN DE CANOPTEK SPYDERS PUEDE COMPRAR MÁS DE UN OBJETO (GH#132). Y UN DESTACAMENTO ALIADO DE TRAITOR GUARD PUEDE FORMAR UN PLATOON (GH#135).",
    line5: "✨ TUS PODERES PSÍQUICOS YA TE DICEN QUÉ HACEN. Lo reportó un jugador y tenía razón: un poder aparecía solo con su nombre — sin alcance, sin valor de lanzamiento, sin objetivo, sin duración ni efecto — en la ficha de unidad, en las hojas de impresión y en la vista de batalla. Eran dos fallos distintos. Las seis DISCIPLINAS GENERALES que puede elegir cualquier psíquico (Biomancia, Adivinación, Piromancia, Telekinesis, Telepatía y la lista siempre conocida) no se guardan con tu facción, así que LOS 31 poderes volvían como «no encontrado» y se imprimían solo con el nombre, en todas las facciones — mientras un poder de tu propio códex una línea más arriba lo mostraba todo. Y la ficha de unidad nunca pasaba los PODERES por el formateador: las oraciones y los pactos recibieron su línea completa hace meses y los poderes se quedaron fuera. Los 808 poderes seleccionables ya resuelven, y LA VISTA DE BATALLA también lleva los datos — justo la pantalla que lees en mitad de la partida era la peor parada. ⚔️ ORKOS: LA BATTLE FORTRESS SE PUEDE ELEGIR. Escalation da tres Lords of War a los orkos y la app solo ofrecía dos — la ficha estaba completa, en un fichero que nada cargaba. 280 puntos, BL 14/13/11, 5 puntos de casco y una capacidad de transporte de 30 que baja 10 por cada cañón grande que le pongas. 🔴 Y la × que cierra este banner ahora es ROJA, porque Unwise no la encontraba nunca.",
    line6: "⚔️ UNA REGLA QUE CINCO FACCIONES TE VENDEN Y NADIE PODÍA COMPRAR. Truesight preguntó cómo gastar el punto por herida del WEBWAY STRIKE éldar, y la respuesta era que no podía: estaba en el códex y la app solo la mostraba como texto. El mismo agujero en cinco sitios: ELDAR y HARLEQUINS «Webway strike», DARK ELDAR «Webway raid», CUSTODES «Lightning strike» y ORKOS «Tellyporta». Cada unidad que califica tiene ya una casilla con lo que cuesta para ESA unidad a su tamaño actual — cambias la escuadra y el precio la sigue — la ficha gana la regla de despliegue, y el límite se aplica: una unidad por cada 1000 puntos EMPEZADOS, o sea tres a 2500. 🛡️ Y UN DESTACAMENTO ALIADO YA NO RECIBE «OBJECTIVE SECURED!», que las Core Rules prohíben de plano. Tu ejército propio no cambia, ni un suplemento como un Asesíno. 💎 UNA RELIQUIA QUE PUEDES COGER DOS VECES YA PUEDE LLEVAR UNA SEGUNDA MEJORA DISTINTA (GH#136). Reportado en la RELIQUIA DEL MUNDO TUMBA nécron: el selector de mejora desaparecía al comprar la primera, y «+ Añadir otro» repetía la misma elección. Ahora sigue abierto, y con él se aplica la regla del códex — «cada mejora es única por ejército» — así que una ya cogida en cualquier parte de tu lista sale en gris y marcada. 19 reliquias del juego funcionan así, no solo la nécron.",
    line7: "📜 CINCO REGLAS QUE TU CÓDEX CONCEDE Y LA APP NO APLICABA — DOS CAMBIAN UNA LISTA GUARDADA, ASÍ QUE REVISA LA TUYA. ADEPTUS SORORITAS: «Shield of Faith: el modelo recibe una salvación invulnerable de 6+» está en el Index y las fichas llevan solo el NOMBRE, así que no había ningún número que leer — 23 DE 28 FICHAS la llevan y TRECE NO MOSTRABAN NINGUNA SALVACIÓN. Ya muestran 6+; las diez que tenían una mejor la conservan. TAU EMPIRE: TACTICAL PHILOSOPHIES ERA GRATIS. La hoja lo cobra en palabras — «10p per 500p game size» — donde todos los demás precios del juego son números, así que entraba por cero; ahora cuesta 20 a 1000 puntos y 50 a 2500, y tu total va a subir. ADEMÁS: el KROOT SHAMAN de Tau cobraba sus +10, convertía al modelo en psíquico y no abría nunca el selector de poderes (su Biomancia y Adivinación son disciplinas compartidas, y solo enseñábamos el panel psíquico a las facciones con una propia). GREY KNIGHTS: el NEMESIS WARDING STAVE da «Parry» a toda la escuadra, en las doce fichas que pueden comprarlo. GUARDIA IMPERIAL: el arma especial extra es tuya solo SI NO FORMAS EQUIPO DE ARMAS PESADAS — llevar ambas queda marcado. Las cinco salieron de un barrido nuevo que pregunta, de cada regla escrita en cada códex, si la app hace algo con ella.",
