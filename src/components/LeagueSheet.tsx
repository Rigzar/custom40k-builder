...
export function LeagueSheet({ event, standings, players, games, onClose }: Props) {
  // The sheet follows the reader's language like the rest of the module. What it CANNOT translate
  // is the organiser's own words — the league name and its description are theirs, as written.
  const t = useT();
  ...
  return createPortal((
    <div id="pv-root" className="fixed inset-0 z-50 overflow-y-auto" style={{ background: '#18171a' }}>
      ...
      <div className="sticky top-0 z-10 flex items-center justify-between gap-2 px-4 py-2 bg-zinc-950 border-b border-zinc-800 print:hidden">
        <span className="text-[11px] uppercase tracking-widest" style={{ color: GOLD }}>{t('evSheet')}</span>
      ...
      <div id="pv-printable" className="max-w-3xl mx-auto px-6 py-8 bg-white text-black">
        ...
        <header className="text-center border-b-2 pb-4 mb-6" style={{ borderColor: GOLD }}>
          ...
          <p className="text-[11px] text-neutral-600 mt-1">
            {event.organiser && <>{tpl(t('evOrganisedBy'), { name: event.organiser })}</>}
        ...
        {event.is_league && (
          <section className="mb-6">
            <h2 className="text-[11px] uppercase tracking-widest mb-2" style={{ color: GOLD_DIM }}>{t('evStandings')}</h2>
            <table className="w-full text-[12px] border-collapse">
              <thead>
                <tr className="border-b border-neutral-300 text-[10px] uppercase tracking-wider text-neutral-500">
                  ...
                  <th className="text-right w-8">{t('evColWins')}</th>
                  <th className="text-right w-8">{t('evColDraws')}</th>
                  <th className="text-right w-8">{t('evColLosses')}</th>
                  <th className="text-right w-10">{t('evColPoints')}</th>
                </tr>
              </thead>
              <tbody>
                {standings.map((s, i) => (
            ...
            <p className="text-[9px] text-neutral-500 mt-1">
              {tpl(t('evScoringNote'), { n: confirmed.length })}
            </p>
          </section>
        )}
        ...
        <section className="mb-6">
          <h2 className="text-[11px] uppercase tracking-widest mb-2" style={{ color: GOLD_DIM }}>
            {t('evParticipants')} ({approved.length})
          </h2>
        ...
        <footer className="mt-8 pt-3 border-t text-[9px] text-neutral-500 text-center" style={{ borderColor: GOLD_DIM }}>
          custom40k-builder.vercel.app · {tpl(t('evSheetGenerated'), { date: new Date().toISOString().slice(0, 10) })}
        </footer>
      </div>
    </div>
  ), document.body);
