...
export function validateArmy(state: ArmyState, data: FactionData, alliedData?: FactionData | null, language: Language = 'en'): ValidationItem[] {
  ...
  // these notes were printed regardless, so the panel showed a green "Clamavus: 1 of 1 unit
  // exempted from the Elite slot (1 per 500pts of game size, 1250pts → max 2)" directly above the
  // error refusing the list. The player believed the exemption existed and that the count was
    ...
    u.option_groups.forEach((g, gi) => {
      ...
      if (g.constraint?.type === 'fixed_max') {
        ...
        if (used > max) {
          items.push({
            type: 'error',
            text: T('valFixedMaxExceeded', { unit: item.unitName, header: g.header.substring(0, 50), used, max }),
          });
        }
      }
    });
